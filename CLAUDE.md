...
| Status | Feature | Why it matters | Implementation notes | Acceptance criteria |
|--------|---------|----------------|----------------------|---------------------|
| `[ ]` | Resale service | Secondary sales are part of the product vision and gateway config already reserves port 5004. | Create service for resale listings, price rules, ownership transfer, escrow/payment handoff, and sale cancellation. | Owner can list an eligible ticket; buyer can purchase resale ticket; original ticket ownership is transferred and auditable. |
| `[ ]` | Waiting list workflow | Supports sold-out events and fair reallocation. | Implement waitlist join/leave, position tracking, notification via `YourTurnInWaitingList`, offer expiry, and reservation handoff. | Sold-out ticket type allows waitlist join; next user receives time-limited offer when inventory returns. |
| `[ ]` | Refund and cancellation workflow | Required for operational support and event cancellation. | Add refund commands, payment refund provider calls, ticket burn command, event cancellation handling, and notification. | Event cancellation can trigger refund flow; refunded tickets cannot verify; blockchain burn command is emitted when needed. |
...
| `[ ]` | Admin/support operations | Operators need safe tools to resolve failed purchases and mints. | Add admin APIs for payment lookup, reservation force-expire, mint retry, verification override, and audit notes. | Admin actions require admin role, are audited, and are covered by authorization tests. |
| `[ ]` | OpenTelemetry tracing across services | Needed to debug distributed purchase flows. | Propagate correlation IDs; configure traces/metrics/log enrichment in all services and workers. | A checkout request can be followed through gateway, ticketing, RabbitMQ, worker, and notification logs/traces. |
| `[~]` | Docker Compose production parity | Current compose files do not run all documented services. | Add service containers as they are implemented; include health checks, networks, env vars, and local blockchain node. | `docker compose -f docker-compose.dev.yml up` starts required infrastructure; full compose can run implemented app services. |
| `[ ]` | Kubernetes manifests for new services | Deployment manifests must match actual services. | Add deployments, services, config maps/secrets, probes, and autoscaling hints for new services. | K8s manifests apply cleanly in a local cluster after required secrets are provided. |
