...
/// <summary>
/// Application service implementation for approval workflow management
/// </summary>
public class ApprovalWorkflowService : IApprovalWorkflowService
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            var workflow = new ApprovalWorkflow
            {
                ...
                ExpiresAt = _dateTimeProvider.UtcNow.Add(template.DefaultExpirationTime),
                Tags = request.Tags,
    ...
    public async Task<PagedResult<ApprovalWorkflow>> GetWorkflowsAsync(
        ...
        CancellationToken cancellationToken = default)
    {
        // Convert from Domain.Services.ApprovalWorkflowFilter to the filter expected by repository
        // The repository likely uses Domain.Common.PagedResult, so we need to map properly
        var repositoryResult = await _workflowRepository.GetPagedAsync(organizationId, filter, pageNumber, pageSize, cancellationToken);
        
        // Map from repository result to Shared.Common.Models.PagedResult
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            workflow.CompletedAt = _dateTimeProvider.UtcNow;
            workflow.CompletionReason = reason;
            workflow.UpdatedAt = _dateTimeProvider.UtcNow;
    ...
    public async Task ProcessExpiredWorkflowsAsync(CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            var expiredWorkflows = await _workflowRepository.GetExpiredWorkflowsAsync(_dateTimeProvider.UtcNow, cancellationToken);
    ...
    public async Task<ApprovalWorkflowStatistics> GetWorkflowStatisticsAsync(
        ...
        CancellationToken cancellationToken = default)
    {
        var domainStats = await _workflowRepository.GetStatisticsAsync(organizationId, fromDate, toDate, cancellationToken);
    ...
    private async Task<bool> CheckTimeConstraintsAsync(CreateApprovalWorkflowRequest request, TimeConstraints constraints)
    {
        ...
        if (constraints.AllowedDaysOfWeek.Any() && !constraints.AllowedDaysOfWeek.Contains(_dateTimeProvider.UtcNow.DayOfWeek))
            return false;

        // Check time window constraints
        if (constraints.AllowedTimeWindow != null)
        {
            var currentTime = _dateTimeProvider.UtcNow.TimeOfDay;
    ...
    private async Task ProcessWorkflowEscalationAsync(
        ...
        CancellationToken cancellationToken)
    {
        var workflowAge = _dateTimeProvider.UtcNow - workflow.CreatedAt;
